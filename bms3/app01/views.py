...
#     # publish = forms.ChoiceField(choices=[('1', '江西软件出版社'), ('2', '人民出版社'), ('3', '新华出版社')])
...
class BookModelForm(forms.ModelForm):
    class Meta:
        ...
        # fields =['title','price','pub_date']  # 对某些字段进行校验
        exclude = ['comment_count', 'poll_count']  # 想要去除的字段
        labels = {
            'title': '书籍名称',
            'price': '价格',
            'pub_date': '日期',
...
def edit(request, edit_book_id):
    book = Book.objects.filter(pk=edit_book_id).first()
    if request.method == 'GET':
...
def query(request):
    ...
    # Book.objects.filter(title__startswith='py').annotate(c=Count('authors'))
    ...
    #     (comment_count__gt=F('poll_count')).values('title')
    # print(ret)
    # 查询评论数大于2倍点赞数的所有书籍名称
    # ret = Book.objects.filter \
    #     (comment_count__gt=F('poll_count') * 2).values('title')
    # print(ret)
    # 给每一本书的价格增加100
    # Book.objects.all().update(price=100 + F('price'))
    # 查询价格大于300或者评论数大于3000的书籍
    # Book.objects.filter(price__gt=300,comment_count__gt=3000) 且
    # 与 & 或 | 非 ~
...
def ajax_del(request, del_book_id):
    res = {'state': True}
    try:
        Book.objects.filter(pk=del_book_id).delete()
