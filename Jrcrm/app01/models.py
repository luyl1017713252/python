...
class UserInfo(AbstractUser):
    ...
    departments = models.ForeignKey(to='Department', on_delete=models.CASCADE, null=True, blank=True)
    ...
    def get_roles_list(self):
        br_l = []
        next = '/assign_permissions/'
        for cls in self.roles.all():
            br_l.append('<a href="/edit/roles/%s/?next=%s">%s</a>' % (cls.pk, next, str(cls)))
        return mark_safe('<br>'.join(br_l))

class Department(models.Model):
    name = models.CharField('部门名称', max_length=32)
    coutent = models.IntegerField('人数')
...
class Role(models.Model):
    ...
    # extra_permissions = models.ForeignKey('Permission', on_delete=models.CASCADE, verbose_name='额外的权限', null=True, blank=True)
    ...
    def get_permissions_list(self):
        br_l = []
        next = '/role/list/'
        for cls in self.permissions.all():
            br_l.append('<a href="/updatepermission/%s/?next=%s">%s</a>' % (cls.pk, next, str(cls.title)))
        return mark_safe('，'.join(br_l))
...
class Permission(models.Model):
    title = models.CharField('权限名', max_length=32)
    url = models.CharField('权限路径', max_length=64)
...
class Customer(models.Model):
    ...
    deal_date = models.DateField('成单日期', null=True, blank=True)
    ...
    def get_status(self):
        ...
        print(status_color[self.status])
...
class ConsultRecord(models.Model):
    ...
    date = models.DateTimeField("跟进日期", auto_now_add=True, null=True)
...
class ClassStudyRecord(models.Model):
    ...
    has_homework = models.BooleanField(default=True, verbose_name="本节有作业", null=True, blank=True)
