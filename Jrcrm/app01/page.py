...
class Pagination(object):
    ...
    def __init__(self, current_page, all_count, request, base_url, per_page_num=2, pager_count=11):
        """
        封装分页相关数据
        ...
        self.page_requst = copy.deepcopy(request.GET)
    ...
    def page_html(self):
        ...
        for i in range(pager_start, pager_end):
            self.page_requst['page'] = i
            if i == self.current_page:
                temp = '<li class="active"><a href="%s?%s">%s</a></li>' % (self.base_url, self.page_requst.urlencode(), i,)
            else:
                temp = '<li><a href="%s?%s">%s</a></li>' % (self.base_url, self.page_requst.urlencode(), i,)
